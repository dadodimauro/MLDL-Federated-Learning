...
class KL_Loss(nn.Module):
    ...
    def forward(self, output_batch, teacher_outputs):
        ...
        # loss_2 = -torch.sum(torch.sum(torch.mul(F.log_softmax(teacher_outputs,dim=1), F.softmax(teacher_outputs,
        # dim=1)+10**(-7))))/teacher_outputs.size(0) print('loss H:',loss_2)
        ...
        # Same result KL-loss implementation loss = T * T * torch.sum(torch.sum(torch.mul(teacher_outputs, torch.log(
        # teacher_outputs) - output_batch)))/teacher_outputs.size(0)
        return loss
